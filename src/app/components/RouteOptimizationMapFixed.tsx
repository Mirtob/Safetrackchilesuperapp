...
export function RouteOptimizationMapFixed({ locations, onRouteOptimized }: RouteOptimizationMapProps) {
  ...
  const calculateOptimizedRoute = useCallback(() => {
    if (!map || locations.length < 2 || typeof google === 'undefined') {
  ...
  useEffect(() => {
    if (locations.length >= 2 && map && isLoaded) {
      calculateOptimizedRoute();
    }
  }, [locations, map, isLoaded]);

  // Función helper para crear el icono del marcador
  const getMarkerIcon = useCallback((taskType: string) => {
    if (typeof google === 'undefined' || !google.maps) return undefined;
    
    return {
      path: google.maps.SymbolPath.CIRCLE,
      scale: 12,
      fillColor: getMarkerColor(taskType),
      fillOpacity: 1,
      strokeColor: 'white',
      strokeWeight: 2
    };
      ...
      <Card className="overflow-hidden border-slate-200 dark:border-zinc-800">
        <LoadScript googleMapsApiKey={GOOGLE_MAPS_API_KEY}>
          ...
          >
            ...
            {/* Marcadores personalizados - solo cuando NO hay direcciones */}
            {!directions && isLoaded && locations.map((location, index) => {
              const markerIcon = getMarkerIcon(location.taskType);
